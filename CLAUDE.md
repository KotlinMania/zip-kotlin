# Claude Code Project Instructions — zip-kotlin

## Project Overview

This is **zip-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`zip`](https://crates.io/crates/zip).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/zip/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/zip>
