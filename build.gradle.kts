...
// kotlin { … }
// ----------------------------------------------------------------------------
// The retired 32-bit watchOS target is intentionally absent by workspace
// product policy (AGENTS.md §5.5.1, effective 2026-05-24). Upstream KGP still
// ships it as Tier 2 — this is a deliberate product decision, not a framework
// deprecation.
...
kotlin {
    ...
    sourceSets {
        commonMain.dependencies {
            implementation(libs.bundles.zip.commonMain)
        }
        commonTest.dependencies {
            implementation(kotlin("test"))
            implementation(libs.anyhow.kotlin)
        }
    }
}
...
detekt {
    ...
    val detektConfig = file("detekt.yml")
...
// it must mirror the kotlin { } target block exactly. The legacy 32-bit watchOS
// target is retired (see §5.5.1); everything else MUST build.
