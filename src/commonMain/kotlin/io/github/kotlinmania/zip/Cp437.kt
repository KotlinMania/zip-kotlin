...
public fun ByteArray.fromCp437(): String {
    val allAscii = this.all { (it.toInt() and 0xFF) < 0x80 }
    return if (allAscii) {
        this.decodeToString()
    } else {
...
/**
 * Converts IBM Code Page 437 encoded byte list to a UTF-8 [String].
 */
 ...
 */
public fun cp437ToChar(input: Int): Char {
    val codePoint =
        when (input and 0xFF) {
            in 0x00..0x7F -> input and 0xFF
            0x80 -> 0x00C7
            0x81 -> 0x00FC
            0x82 -> 0x00E9
            0x83 -> 0x00E2
